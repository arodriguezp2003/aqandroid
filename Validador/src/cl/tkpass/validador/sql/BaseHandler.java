...
public class BaseHandler extends SQLiteOpenHelper{
	...
	public List<Brazaletes> BrazaleteEnviado(String Data) throws ParseException
	{
		...
		Cursor c = db.rawQuery("SELECT * FROM BRAZALETES WHERE  Enviado =? ", where); 
		if (c.moveToFirst()) 
		{
			do{
	...
	public void Enviado(int id) {
		// TODO Auto-generated method stub
		SQLiteDatabase db = this.getWritableDatabase();
		db.execSQL("UPDATE BRAZALETES SET Enviado=1 WHERE id=" + id);
	}
}
